...
public class RestApiController {
	...
	// 아래 포스트에 특정 번호 입력하면 데이터를 가져오는 메소드 구현, 필요시 글 미리 등록
	// <문제점>
	// 본글에 참조키로 연결된 리뷰데이터가 존재할 경우 응답값이 무한반복(무한참조)이 됨 -> 본문 따로, 리뷰 따로(페이징)
	// 본글만 있으면 문제 없음
	...
	@Parameters({
		@Parameter(name = "id", description = "포스트 글의 고유번호 값", example = "1")
	})
	// 통신 성공 후, 응답 데이터에 대한 의미(도)를 코드로 전달할 떄, 그 값을 체크하는 용도
	// 통신 프로토콜표 = 전문 = TR
	@ApiResponses( value = {
	...
	// 그룹내 api별 구분 용도 사용
	@Operation(summary = "포스트 정보 조회(헤더 조작 가능)", 
			   description = "포스트 내용 조회 후 응답시 헤더를 커스텀하게 조정할 수 있다")	
	// ResponseEntity는 사용하면 -> 응답 헤더 조정 가능함
	public ResponseEntity<PostForm> post3(@PathVariable("id") Integer id) {
		PostForm p = this.postService.getOnePostForm(id);
